...
public class OrderService {
	...
	private Order saveOrderDetails(OrderDomain order) throws Exception {
		...
		if (!carts.isEmpty()) {
			...
			order1.setTotal(totals.stream().mapToDouble(a -> a).sum());
	...
	public Order updateOrder(OrderDomain orderDomain, Long orderId) throws Exception {
		...
		if (orderRepository.findOne(orderId) != null) {
			...
			order.setAddress(orderDomain.getAddress());
			order.setOrderId(orderId);
			order.setUserEmail(orderDomain.getEmailId());
			order.setUserName(orderDomain.getUsername());
			Set<Cart> carts = new HashSet<>();

			ArrayList<Double> totals = new ArrayList<>();

			Set<ItemDomain> list = orderDomain.getItemDomain();
			for (ItemDomain item : list) {
			...
			if (carts.isEmpty()) {
				throw new Exception("Items are out of stock currently");
			}
			order.setItems(carts);
			order.setTotal(totals.stream().mapToDouble(a -> a).sum());
